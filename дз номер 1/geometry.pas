...
function RadiusT(const R : Triangle) : Real;
function RadiusO(const Ro : Triangle) : Real;
...
function RadiusO(const Ro : Triangle) : Real;
 var
  ...
  AB, BC, CA : Real;
begin
  AB := GetDistance (Ro.A,Ro.B);
  BC := GetDistance (Ro.B,Ro.C);
  CA := GetDistance (Ro.C,Ro.A);
  S := GetTrSq(Ro);
  RadiusO := AB*BC*CA/4/S;
end;
